...
export class TCPServer {
  ...
  private handleMessage(peerId: string, socket: any, chunk: Buffer): void {
    ...
    if (state.isHTTP || this.isHTTPRequest(chunk.toString('utf8'))) {
      this.handleHTTPData(peerId, socket, chunk).catch((err) => {
  ...
  private async handleHTTPData(peerId: string, socket: any, chunk: Buffer): Promise<void> {
    const state = this.connectionStates.get(peerId);
    if (!state) {
      return;
    }
    ...
    while (true) {
      ...
      if (parsed.needsMoreData) {
        if (!state.continueSent) {
          ...
          if (sepIdx !== -1) {
            const headerBlock = state.buffer.slice(0, sepIdx).toString('utf8').toLowerCase();
